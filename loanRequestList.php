...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['accept'])) {
        ...
        $sql = "UPDATE loan_list SET first_name=?, last_name=?, address=?, contact=?, loan_amount=?,total_balance=?, remaining_balance=?, due_date=?, pay_date=?, penalty_date=?, status=? WHERE id=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssdddssssi", $firstName, $lastName, $address, $contact, $loanAmount, $remainingBalance, $remainingBalance, $dueDate, $payDate, $payDate, $status, $id);
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        echo "<tr class='editableRow' data-id='" . $row['id'] . "' data-firstname='" . $row['first_name'] . "' data-lastname='" . $row['last_name'] . "' data-address='" . $row['address'] . "' data-contact='" . $row['contact'] . "' data-loanamount='" . $row['loan_amount'] . "' data-remainingbalance='" . "' data-duedate='" . $row['due_date'] . "' data-duration='" . $row['duration'] . "'>";
        ...
        <div id="editForm" class="popupForm">
            <div class="formContainer">
                ...
                <form id="form" method="POST">
                    ...
                    <input type="number" id="duration" name="duration" step="0.01" hidden>
